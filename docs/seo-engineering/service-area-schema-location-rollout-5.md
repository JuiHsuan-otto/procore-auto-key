# Service-area schema location rollout 5
...
The `location-page-shared-business-batch-5` stage contains:

1. `new-taipei-car-key.html`
2. `penghu-car-key.html`
3. `pingtung-car-key.html`
...
The governance register changes the local candidate remainder from 10 files／10 nodes to 7 files／7 nodes. This deletion does not verify any business service area and does not authorize changing the remaining location pages.
...
- The preceding 90 local HTML deltas must retain the same exact-delta protection.
...
- Governance must report 7 files／7 typed shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head`: passed across 134 registered schema pages; all 93 local HTML deltas match their exact registered transformation and the comparator reported 0 errors.
- `npm run validate:governance`: passed with 7 files／7 typed shared-business nodes remaining, 2 known warnings, and 0 errors.
