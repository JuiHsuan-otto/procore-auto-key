# Service-area schema guide rollout 1

Date: 2026-07-19
Base: `main@42b99ac1a16908edbe744ff003b7c9607290b8d3`
Depends on: the three earlier service-area evidence rollout records

## Outcome

The evidence-gated shared-business rule was extended to five related owner preparation／diagnostic guides:
...
Each page had exactly one typed shared `https://www.carkey.com.tw/#business` node with `areaServed` and no separate `Service.areaServed` node. Only that unsupported shared-business property was removed. The controlled remainder moves from 119 files／119 nodes to 114 files／114 nodes.

Subsequent status: the separately documented second through fourth guide rollouts close the registered guide scope and move the current governed remainder to 100 files／100 nodes. The 114 count above remains the immutable post-first-guide observation.

## Business value

These guides support high-intent users preparing details or diagnosing a key issue. Removing unverified business-wide coverage from their shared identity prevents search engines from receiving a claim that conflicts with the governed source of truth, without reducing the guides' useful content or contact paths.

## Scope and exclusions

- No visible copy, service-area statement, title, description, canonical, URL, CTA, navigation, FAQ, image, or CSS changes.
- No Service node existed on these five pages, so no page-specific `Service.areaServed` was changed.
- The remaining fifteen registered guide pages, case pages, location pages, and utility pages are outside this batch.
...
- Governance gates enforce four ordered stages and the exact 114-file／114-node remainder.
