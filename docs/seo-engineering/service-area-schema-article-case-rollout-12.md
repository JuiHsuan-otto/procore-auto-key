# Service-area schema article-case rollout 12
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Four remaining Volkswagen article pages still attach regional coverage to the typed shared `#business` node. A case location or article subject does not prove business-wide service coverage. Omitting this optional shared property reduces unsupported entity claims without changing customer-facing evidence or contact paths.

This is P0 schema-integrity work. It closes the Volkswagen article scope in one reversible four-page batch while keeping every accumulated local page delta protected by an exact HEAD comparator.

## Scope

The `article-case-page-batch-12` stage contains:

1. `article-vw-ignition-repair.html`
2. `article-vw-pointer-xianxi-rescue.html`
3. `article-vw-t5-kaohsiung-rescue.html`
4. `article-vw-t6-2022-taipei-smartkey.html`

Only `areaServed` on the typed shared node with `@id=https://www.carkey.com.tw/#business` is removed. All four pages have no page-specific `Service.areaServed` and no second LocalBusiness node. Visible locations remain unchanged and are not interpreted as verified business-wide coverage.

The governance register changes the local candidate remainder from 32 files／32 nodes to 28 files／28 nodes. Since earlier local batches remain uncommitted against the same HEAD, the current HEAD-delta comparator intentionally contains all 72 local HTML deltas. The Volkswagen article scope is now closed; this does not close or verify any broader service-area scope.
...
- The four HTML files may differ from `HEAD` only by their exact registered typed shared-business `areaServed` removal.
- The preceding sixty-eight local HTML deltas must retain the same exact-delta protection.
...
- Governance must report 28 files／28 typed shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head` passed: 134 schema pages, 0 errors; all 72 local HTML deltas are exact registered typed shared-business removals from the same HEAD.
- `npm run validate:governance` passed: 28 files／28 typed shared-business nodes remaining, 2 known warnings, 0 errors.
...
- Local HTTP smoke passed on all four `.html` files: HTTP 200, one H1, parseable JSON-LD, one typed shared business node, zero typed shared-business `areaServed`, zero page-specific `Service.areaServed`, and zero additional LocalBusiness nodes.
...
Risk is limited to omitting one optional structured-data property from four typed shared business nodes and registering the exact local delta. Regression gates still cover every previously governed file. Rollback is the exact inverse of the four JSON-LD deletions plus removal of this stage from the migration source, comparator, governance, and documentation. No URL, visible content, database, or Production data migration is involved.

## Suggested commit boundary

`fix(seo): close Volkswagen article service-area cleanup`
