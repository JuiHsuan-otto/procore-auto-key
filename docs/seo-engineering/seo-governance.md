...
- `data/business-entity.json` is the only future source for business identity fields. A field is schema-eligible only when `status=verified`, `publish=true`, and source/reviewer/date evidence exist. An unpublished field must not appear on the typed shared business node except as an explicitly counted legacy migration; the `serviceArea` pilot, service-page rollout, brand／model rollout, four guide rollouts, three-page case pilot, complete twelve-page BMW case scope, twelve article-case batches, the first three-page hub／utility batch, the three-page non-location closure, six three-page location batches, and the final four-page location closure reduce the typed shared-business remainder to zero in the current local candidate. Page-specific `Service.areaServed` and visible location content remain outside this shared-node deletion and do not verify a business-wide service area. A separate legacy LocalBusiness node on the central-Taiwan rescue article remains outside this typed shared-node migration and is counted independently rather than silently reclassified.
- The exact schema comparator uses immutable pre-current-rollout commit `2d60534b779d29562462130d953e619b2e82005f`, recorded as `comparison_base_sha`. It must not follow mutable `HEAD` or be advanced merely to make validation pass. `npm run validate:schema-rollout` runs this comparison by default; any future baseline transition requires an explicit governance decision and evidence review.
