# Service-area schema article-case rollout 7
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Five Skoda／Taichung／Toyota article-case pages still attach regional coverage to the typed shared `#business` node. A case or article location does not prove business-wide service coverage. Omitting this optional shared property reduces unsupported entity claims without changing customer-facing evidence or contact paths.
...
The `article-case-page-batch-7` stage contains:

1. `article-skoda-kamiq-crushed-key.html`
2. `article-taichung-lost-key.html`
3. `article-taichung-lost-key-preparation.html`
4. `article-toyota-altis-2020-yuanlin-akl.html`
5. `article-toyota-rav4-linkou-rescue.html`

Only `areaServed` on the typed shared node with `@id=https://www.carkey.com.tw/#business` is removed. Source inspection found no page-specific `Service.areaServed` or second LocalBusiness node on these five pages. Visible locations remain unchanged and are not interpreted as verified business-wide coverage.

The governance register changes the local candidate remainder from 57 files／57 nodes to 52 files／52 nodes. Since earlier local batches remain uncommitted against the same HEAD, the current HEAD-delta comparator intentionally contains all 48 local HTML deltas.
...
- The preceding forty-three local HTML deltas must retain the same exact-delta protection.
- All 134 registered schema pages must parse and retain exactly one typed shared business node.
- Governance must report 52 files／52 typed shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head` passed: 134 schema pages, 0 errors; all 48 local HTML deltas are exact registered typed shared-business removals from the same HEAD.
- `npm run validate:governance` passed: 52 files／52 typed shared-business nodes remaining, 2 known warnings, 0 errors.
...
- Local HTTP smoke passed on all five `.html` files: HTTP 200, one H1, parseable JSON-LD, one typed shared business node, zero typed shared-business `areaServed`, no page-specific `Service.areaServed`, and no other LocalBusiness node.
...
`fix(seo): continue Skoda Taichung Toyota service-area cleanup`
