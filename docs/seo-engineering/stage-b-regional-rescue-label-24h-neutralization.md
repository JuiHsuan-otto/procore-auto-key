# Stage B: regional rescue-label availability-claim neutralization

Status: implemented and verified locally; not committed, pushed, published, or deployed.

## Problem, priority, and scope

Human input H07 still lacks staffing or on-call evidence supporting public `24H`, `24小時`, `全年無休`, or `全天候` promises. This P0 batch removes the unsupported time promise while retaining useful location and telephone information on five pages:
...
The visible CTAs become `和美救援`, `草屯救援`, `中科救援`, `高雄救援`, and `霧峰救援`, each retaining `0909-277-670`. Initial browser verification also confirmed a pre-existing mobile overflow on all five pages: the two CTA anchors extended to 405px inside a 375px content viewport because their flex container could not wrap. The same container gains only `flex-wrap`, allowing the existing anchors to wrap on narrow screens. Telephone destinations, LINE CTAs, schema, URLs, and unrelated content remain unchanged. Existing `priceRange` debt is out of scope.

## Controlled migration

- Original baseline: 39 files / 60 occurrences.
- Prior five stages: removed 23 files / 41 occurrences.
- This regional rescue-label batch: removed 5 files / 5 occurrences.
- Expected remainder: 11 files / 14 occurrences.
- `openingHours` remains null, unverified, and unpublished.

The governance validator records this as the sixth migration stage. Each location-specific anchor and the single container-class addition have exact before/after pairs in the per-file HEAD comparator.

## Pre-write production baseline

This is a point-in-time observation, not a permanent test truth:

- Captured: `2026-07-17T01:13:09+08:00` (Asia/Taipei).
...
- `node scripts/validate-seo-governance.mjs --compare-head --self-test`: exact 11-file / 14-occurrence remainder, four negative gates, 0 errors.
...
- Initial browser run identified the pre-existing shared-template overflow at all five mobile pages: `scrollWidth 405 / viewportWidth 375`.
- After the allowlisted `flex-wrap` addition, all five pages at 1440×900 and 390×844 viewport overrides have one H1, one expected regional phone CTA, zero target claims, zero console warnings/errors, and `scrollWidth === viewportWidth` at both sizes.
...
- Captured: `2026-07-17T01:18:20+08:00` (Asia/Taipei).
...
Rollback is the inverse of the five registered anchor-text replacements and five `flex-wrap` class additions, plus removal of the sixth migration stage, validator registrations, and this evidence document. It must not be used merely to restore an unsupported availability promise.

Suggested future commit boundary: `fix(content): neutralize regional 24H labels and mobile CTA overflow`.
