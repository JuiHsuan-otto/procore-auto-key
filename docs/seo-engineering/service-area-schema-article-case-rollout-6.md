# Service-area schema article-case rollout 6
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Five Nissan／Porsche／Range Rover article-case pages still attach regional coverage to the typed shared `#business` node. A case location does not prove business-wide service coverage. Omitting this optional shared property reduces unsupported entity claims without changing customer-facing evidence or contact paths.
...
The `article-case-page-batch-6` stage contains:

1. `article-nissan-kicks-zhongke-rescue.html`
2. `article-porsche-cayenne-hemei.html`
3. `article-porsche-lost-key-rescue.html`
4. `article-porsche-panamera-wugu-akl.html`
5. `article-range-rover-huatan-service.html`
...
The governance register changes the local candidate remainder from 62 files／62 nodes to 57 files／57 nodes. Since earlier local batches remain uncommitted against the same HEAD, the current HEAD-delta comparator intentionally contains all 43 local HTML deltas.
...
- The preceding thirty-eight local HTML deltas must retain the same exact-delta protection.
- All 134 registered schema pages must parse and retain exactly one typed shared business node.
- Governance must report 57 files／57 typed shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head` passed: 134 schema pages, 0 errors; all 43 local HTML deltas are exact registered typed shared-business removals from the same HEAD.
- `npm run validate:governance` passed: 57 files／57 typed shared-business nodes remaining, 2 known warnings, 0 errors.
...
`fix(seo): continue Nissan Porsche Range Rover service-area cleanup`
