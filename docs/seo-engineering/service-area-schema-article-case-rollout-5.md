# Service-area schema article-case rollout 5
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Five Kia／Lexus／Mazda／Mini article-case pages still attach regional coverage to the typed shared `#business` node. A guide label or case location does not prove business-wide service coverage. Omitting this optional shared property reduces unsupported entity claims without changing customer-facing evidence or contact paths.
...
The `article-case-page-batch-5` stage contains:

1. `article-kia-picanto-shengang-akl.html`
2. `article-lexus-taichung-service.html`
3. `article-mazda-cx3-hemei-smartkey.html`
4. `article-mazda-wufeng-rescue.html`
5. `article-mini-clubman-beidou-akl.html`
...
The governance register changes the local candidate remainder from 67 files／67 nodes to 62 files／62 nodes. Since earlier local batches remain uncommitted against the same HEAD, the current HEAD-delta comparator intentionally contains all 38 local HTML deltas.
...
- The preceding thirty-three local HTML deltas must retain the same exact-delta protection.
- All 134 registered schema pages must parse and retain exactly one typed shared business node.
- Governance must report 62 files／62 typed shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head` passed: 134 schema pages, 0 errors; all 38 local HTML deltas are exact registered typed shared-business removals from the same HEAD.
- `npm run validate:governance` passed: 62 files／62 typed shared-business nodes remaining, 2 known warnings, 0 errors.
...
`fix(seo): continue Kia Lexus Mazda Mini service-area cleanup`
