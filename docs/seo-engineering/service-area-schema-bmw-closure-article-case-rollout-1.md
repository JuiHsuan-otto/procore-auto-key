# Service-area schema BMW closure and article-case rollout 1
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. The final two registered BMW case pages and three same-template Benz case pages still attach regional coverage to the shared `#business` node. Individual case locations do not establish business-wide coverage, so omitting this optional property reduces unsupported entity claims while retaining any page-specific service evidence.

This is P0 schema integrity work. It closes the registered BMW case scope and begins the broader article-case rollout without changing customer-facing content or the enquiry journey.

## Scope

The `bmw-case-page-closure` stage contains:

1. `article-bmw-x3-linkou-auction-akl.html`
2. `article-bmw-x5-battery-fix.html`

The `article-case-page-batch-1` stage contains:

1. `article-benz-c300-wuqi.html`
2. `article-benz-glc300-yuanlin-service.html`
3. `article-benz-glk-miaoli-akl.html`

Only the shared typed business node loses `areaServed`. The BMW X3 page also contains a separate page-specific `Service.areaServed` value for its case location; that property remains byte-for-byte unchanged and is not interpreted as verified business-wide coverage. The other four pages have no page-specific `Service.areaServed`.

The governance register changes the local candidate remainder from 87 files／87 nodes to 82 files／82 nodes and marks the twelve-page registered BMW case scope closed. Because all prior local batches remain uncommitted against the same HEAD, the current HEAD-delta comparator intentionally contains all 18 local HTML deltas.
...
- No page-specific `Service.areaServed` or visible location copy is removed, generalized, or promoted to business evidence.
...
- BMW X3 must retain its page-specific `Service.areaServed` exactly.
- The preceding thirteen local case deltas must retain the same exact-delta protection.
- All 134 registered schema pages must parse and retain exactly one typed shared business node.
- Governance must report 82 files／82 shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head` passed: 134 schema pages, 0 errors; all 18 local case-page deltas are exact removals from the shared HEAD.
- `npm run validate:governance` passed: 82 files／82 shared-business nodes remaining, 2 known warnings, 0 errors.
...
- Local HTTP smoke passed on all five pages: HTTP 200, one H1, parseable JSON-LD, one typed shared business node, and zero shared-business `areaServed` properties per page.
- BMW X3 retained exactly one page-specific `Service.areaServed` value, `新北市林口區`; the other four pages retained no `Service.areaServed`, matching their baselines.
- The temporary preview was bound only to `127.0.0.1` and stopped after validation.

## Point-in-time Production baseline

Captured at `2026-07-19T14:47:26+08:00` (Asia/Taipei). A live `git ls-remote` immediately beforehand confirmed `refs/heads/main` at `f616264a67a40474ed159137d1cc7c0c450a2de6`. This is an immutable observation, not a permanent test truth; later Production changes must be reported as drift rather than rewriting this record or Production.
...
Risk is limited to omitting one optional structured-data property from five shared business nodes and registering the exact local delta. The page-specific BMW X3 service location is explicitly preserved. Regression gates still cover every previously governed file. Rollback is the exact inverse of the five JSON-LD deletions plus removal of these two stages from the migration source, comparator, governance, and documentation. No URL, visible content, database, or Production data migration is involved.

## Suggested commit boundary

`fix(seo): close BMW service-area debt and start article cases`
