# Service-area schema article-case rollout 10
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Five Toyota／Volvo article-case pages still attach regional coverage to the typed shared `#business` node. A case location does not prove business-wide service coverage. Omitting this optional shared property reduces unsupported entity claims without changing customer-facing evidence or contact paths.
...
The `article-case-page-batch-10` stage contains:

1. `article-toyota-rav4-fengyuan-akl.html`
2. `article-toyota-rav4-nantou-rescue.html`
3. `article-toyota-sienta-hemei-service.html`
4. `article-toyota-vios-2019-changhua-akl.html`
5. `article-volvo-keyless-lost-rescue.html`
...
The governance register changes the local candidate remainder from 42 files／42 nodes to 37 files／37 nodes. Since earlier local batches remain uncommitted against the same HEAD, the current HEAD-delta comparator intentionally contains all 63 local HTML deltas.
...
- The preceding fifty-eight local HTML deltas must retain the same exact-delta protection.
...
- Governance must report 37 files／37 typed shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head` passed: 134 schema pages, 0 errors; all 63 local HTML deltas are exact registered typed shared-business removals from the same HEAD.
- `npm run validate:governance` passed: 37 files／37 typed shared-business nodes remaining, 2 known warnings, 0 errors.
...
`fix(seo): continue Toyota Volvo service-area cleanup`
