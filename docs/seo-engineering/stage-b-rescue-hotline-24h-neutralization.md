# Stage B: rescue-hotline availability-claim neutralization

Status: implemented and verified locally; not committed, pushed, published, or deployed.

## Problem, priority, and scope

Human input H07 still lacks staffing or on-call evidence supporting public `24H`, `24小時`, `全年無休`, or `全天候` promises. This P0 batch lowers expectation and brand-trust risk on five pages using the exact same `24H 救援專線` CTA:
...
Each page receives one exact replacement from `24H 救援專線` to `救援專線`. The telephone destination, adjacent LINE link, CTA structure, schema, URL, CSS classes, and unrelated content remain unchanged. Existing `priceRange` debt is out of scope.
...
- Contact-label batch: removed 4 files / 11 occurrences.
- This rescue-hotline batch: removed 5 files / 5 occurrences.
- Expected remainder: 20 files / 23 occurrences.
- `openingHours` remains null, unverified, and unpublished.

The governance validator records this as the fourth migration stage. Its per-file HEAD comparator reconstructs every affected page using only the registered replacement and rejects any unrelated byte change.

## Pre-write production baseline

This is a point-in-time observation, not a permanent test truth:

- Captured: `2026-07-17T01:01:28+08:00` (Asia/Taipei).
...
- `node scripts/validate-seo-governance.mjs --compare-head --self-test`: exact 20-file / 23-occurrence remainder, four negative gates, 0 errors.
...
- Browser, all five pages at 1440×900 and 390×844 viewport overrides: one H1, one visible `救援專線` CTA, zero visible target claims, zero horizontal overflow, and zero console warnings/errors on every page.
...
The first comparator attempt correctly rejected an ambiguous substring allowlist because `救援專線` is contained within `24H 救援專線`. The final guard uses the complete SVG-adjacent CTA text block from `HEAD`, passed on all five pages, and did not weaken comparison rules.

## Final production drift observation

- Captured: `2026-07-17T01:04:39+08:00` (Asia/Taipei).
...
Rollback is the inverse of the five registered text replacements plus removal of the fourth migration stage, validator registrations, and this evidence document. It must not be used merely to restore an unsupported availability promise.

Suggested future commit boundary: `fix(content): neutralize unsupported 24H rescue hotline labels`.
