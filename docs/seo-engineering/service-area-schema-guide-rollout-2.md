# Service-area schema guide rollout 2

Date: 2026-07-19
Base: `main@42b99ac1a16908edbe744ff003b7c9607290b8d3`
Depends on: the four earlier service-area evidence rollout records

## Outcome

The evidence-gated shared-business rule was extended to five related Keyless／lost-key／smart-key guides:
...
Each page had exactly one typed shared `https://www.carkey.com.tw/#business` node with `areaServed` and no separate `Service.areaServed` node. Only that unsupported shared-business property was removed. The controlled remainder moves from 114 files／114 nodes to 109 files／109 nodes.

Subsequent status: the separately documented third and fourth guide rollouts close the registered guide scope and move the current governed remainder to 100 files／100 nodes. The 109 count above remains the immutable post-second-guide observation.

## Business value

These pages cover high-intent troubleshooting, lost-key preparation, and owner guidance. Their shared business identity should not publish a service-coverage claim that the governed business source explicitly marks unverified and unpublished. Omitting the optional property improves entity-data integrity without changing the usefulness or conversion paths of the guides.
...
- Remaining guide, case, location, language, and utility pages are outside this batch.
...
- Governance gates enforce five ordered stages and the exact 109-file／109-node remainder.
