# Service-area schema location rollout 3
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Three more location pages still attached an identical page-specific place list both to the shared `#business` node and to the page's own `Service` node. The page content and `Service.areaServed` remain useful page-specific semantics, but they do not independently verify a business-wide service area.
...
The `location-page-shared-business-batch-3` stage contains:

1. `kaohsiung-car-key.html`
2. `keelung-car-key.html`
3. `kinmen-car-key.html`
...
The governance register changes the local candidate remainder from 16 files／16 nodes to 13 files／13 nodes. This deletion does not verify any business service area and does not authorize changing the remaining location pages.
...
- The preceding 84 local HTML deltas must retain the same exact-delta protection.
...
- Governance must report 13 files／13 typed shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head`: passed across 134 registered schema pages; all 87 local HTML deltas match their exact registered transformation and the comparator reported 0 errors.
- `npm run validate:governance`: passed with 13 files／13 typed shared-business nodes remaining, 2 known warnings, and 0 errors.
