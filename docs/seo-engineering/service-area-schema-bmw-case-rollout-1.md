# Service-area schema BMW case rollout 1
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Five BMW case pages still publish regional coverage on the shared `#business` node even though those case locations are not evidence of business-wide coverage. Omitting the optional property reduces unsupported entity claims and makes later schema maintenance evidence-gated.

This is P0 schema integrity work. It lowers compliance and search-engine consistency risk without changing customer-facing content or the enquiry path.

## Scope

Only the shared typed business node loses `areaServed` on:

1. `article-bmw-118-2016-changhua-akl.html`
2. `article-bmw-118-beitun-akl.html`
3. `article-bmw-218d-2016-yunlin-akl.html`
4. `article-bmw-220i-2015-yunlin-akl.html`
5. `article-bmw-3series-huatan-rescue.html`

Each page has one typed shared business node and no page-specific `Service.areaServed`. The governance register adds the five-page batch and changes the local candidate remainder from 97 files／97 nodes to 92 files／92 nodes. The current HEAD-delta comparator intentionally contains the preceding three-page uncommitted pilot plus these five pages because all eight page deltas are still local relative to the same HEAD.
...
- The preceding three-page pilot must retain the same exact-delta protection.
- All 134 registered schema pages must parse and retain exactly one typed shared business node.
- Governance must report 92 files／92 shared-business nodes remaining and reject reintroduction on all governed pages.
...
- `npm run validate:schema-rollout -- --self-test --compare-head` passed: 134 schema pages, 0 errors; both the prior three-page pilot and this five-page batch are exact deltas from the shared HEAD.
- `npm run validate:governance` passed: 92 files／92 shared-business nodes remaining, 2 known warnings, 0 errors.
...
- Local HTTP smoke passed on all five pages: HTTP 200, one H1, parseable JSON-LD, one typed shared business node, and zero shared-business `areaServed` properties per page. Three pages contain two JSON-LD blocks and two contain three, matching their existing templates.
- The temporary preview was bound only to `127.0.0.1` and stopped after validation.

## Point-in-time Production baseline

Captured at `2026-07-19T12:47:02+08:00` (Asia/Taipei). A live `git ls-remote` immediately beforehand confirmed `refs/heads/main` at `f616264a67a40474ed159137d1cc7c0c450a2de6`. This is an immutable observation, not a permanent test truth; later Production changes must be reported as drift rather than rewriting this record or Production.
