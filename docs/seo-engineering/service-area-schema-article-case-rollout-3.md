# Service-area schema article-case rollout 3
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Five Ford／Honda case pages still attach regional coverage to the shared `#business` node. A case location does not prove business-wide coverage. Omitting the optional shared property reduces unsupported entity claims without altering visible case evidence or customer contact paths.

This is P0 schema-integrity work. It continues the small, reversible rollout and preserves exact HEAD comparisons across all accumulated local page deltas.

## Scope

The `article-case-page-batch-3` stage contains:

1. `article-ford-focus-puli-akl.html`
2. `article-ford-kuga-huatan-akl.html`
3. `article-honda-cbr650-2022-kaohsiung-akl.html`
4. `article-honda-fit-2018-kaohsiung-akl.html`
5. `article-honda-fit-tanaka-rescue.html`
...
The governance register changes the local candidate remainder from 77 files／77 nodes to 72 files／72 nodes. Because prior local batches remain uncommitted against the same HEAD, the current HEAD-delta comparator intentionally contains all 28 local HTML deltas.
...
- The preceding twenty-three local HTML deltas must retain the same exact-delta protection.
- All 134 registered schema pages must parse and retain exactly one typed shared business node.
- Governance must report 72 files／72 shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head` passed: 134 schema pages, 0 errors; all 28 local HTML deltas are exact registered shared-business removals from the same HEAD.
- `npm run validate:governance` passed: 72 files／72 shared-business nodes remaining, 2 known warnings, 0 errors.
...
`fix(seo): continue Ford and Honda service-area cleanup`
