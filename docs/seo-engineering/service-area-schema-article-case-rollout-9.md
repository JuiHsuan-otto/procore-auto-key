# Service-area schema article-case rollout 9
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Five Peugeot／Skoda／Toyota article-case pages still attach regional coverage to the typed shared `#business` node. A case location does not prove business-wide service coverage. Omitting this optional shared property reduces unsupported entity claims without changing customer-facing evidence or contact paths.
...
The `article-case-page-batch-9` stage contains:

1. `article-peugeot-508sw-zhubei-smartkey.html`
2. `article-skoda-kodiaq-yunlin-rescue.html`
3. `article-toyota-altis-2016-taichung-akl.html`
4. `article-toyota-altis-qingshui-rescue.html`
5. `article-toyota-auris-linkou-auction-akl.html`

Only `areaServed` on the typed shared node with `@id=https://www.carkey.com.tw/#business` is removed. The Toyota Auris page separately contains a page-specific `Service.areaServed: 新北市林口區` and its nested provider LocalBusiness; both are preserved and counted independently. The other four pages have no page-specific `Service.areaServed` or second LocalBusiness node. Visible locations remain unchanged and are not interpreted as verified business-wide coverage.

The governance register changes the local candidate remainder from 47 files／47 nodes to 42 files／42 nodes. Since earlier local batches remain uncommitted against the same HEAD, the current HEAD-delta comparator intentionally contains all 58 local HTML deltas.
...
- The Toyota Auris page-specific Service and provider entity are not removed, generalized, or promoted to business evidence.
...
- The preceding fifty-three local HTML deltas must retain the same exact-delta protection.
- All 134 registered schema pages must parse and retain exactly one typed shared business node.
- The Toyota Auris page must retain one page-specific `Service.areaServed` and one nested provider LocalBusiness distinct from the shared business node.
- Governance must report 42 files／42 typed shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head` passed: 134 schema pages, 0 errors; all 58 local HTML deltas are exact registered typed shared-business removals from the same HEAD.
- `npm run validate:governance` passed: 42 files／42 typed shared-business nodes remaining, 2 known warnings, 0 errors.
...
- Local HTTP smoke passed on all five `.html` files: HTTP 200, one H1, parseable JSON-LD, one typed shared business node, and zero typed shared-business `areaServed`. The Toyota Auris page retained one page-specific `Service.areaServed` and one nested provider LocalBusiness; the other four pages retained neither.
...
Risk is limited to omitting one optional structured-data property from five typed shared business nodes and registering the exact local delta. Regression gates still cover every previously governed file and explicitly preserve the Toyota Auris Service/provider graph. Rollback is the exact inverse of the five JSON-LD deletions plus removal of this stage from the migration source, comparator, governance, and documentation. No URL, visible content, database, or Production data migration is involved.

## Suggested commit boundary

`fix(seo): continue Peugeot Skoda Toyota service-area cleanup`
