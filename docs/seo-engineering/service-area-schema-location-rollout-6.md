# Service-area schema location rollout 6
...
The governed business source keeps `serviceArea` empty, unverified, and unpublished. Three more location pages attached an identical page-specific place list both to the shared `#business` node and to the page's own `Service` node. The page content and `Service.areaServed` remain useful page-specific semantics, but they do not independently verify a business-wide service area.

This is P0 schema-integrity work. It removes an unsupported shared-business claim while preserving each local landing page and all customer contact paths.

## Scope

The `location-page-shared-business-batch-6` stage contains:

1. `taichung-car-key.html`
2. `tainan-car-key.html`
3. `taipei-car-key.html`
...
The governance register changes the local candidate remainder from 7 files／7 nodes to 4 files／4 nodes. This deletion does not verify any business service area and does not authorize changing the remaining location pages.
...
- The preceding 93 local HTML deltas must retain the same exact-delta protection.
...
- Governance must report 4 files／4 typed shared-business nodes remaining and reject reintroduction on every governed page.
...
- `npm run validate:schema-rollout -- --self-test --compare-head`: passed across 134 registered schema pages; all 96 local HTML deltas match their exact registered transformation and the comparator reported 0 errors.
- `npm run validate:governance`: passed with 4 files／4 typed shared-business nodes remaining, 2 known warnings, and 0 errors.
...
Baseline summary: 7 checked, 7 match, 0 drift. The end-of-run `git ls-remote` check at `2026-07-20T10:07:31+08:00` still returned `f616264a67a40474ed159137d1cc7c0c450a2de6`; remote `main` did not move during the batch. A separate read-only comparison captured at `2026-07-20T10:07:45+08:00` again returned 7 match／0 drift, so the observed Production bodies also remained aligned with that SHA through the end of the batch. No Production mutation, click, deployment, or rewrite of an earlier baseline occurred.
