...
const contractRoot = path.resolve("contracts/publication-action/v1");
...
const expected = new Map(checksumFile.trim().split(/\r?\n/).map((line) => {
  ...
  if (!match) throw new Error("Malformed publication-action CHECKSUMS.sha256");
...
for (const [name, digest] of expected) {
  ...
  if (actual !== digest) throw new Error(`Publication-action checksum mismatch: ${name}`);
...
if (index !== -1) {
  ...
  if (JSON.stringify(names) !== JSON.stringify(referenceNames)) throw new Error("Publication-action mirror file set drift");
  for (const name of names) {
    const local = await readFile(path.join(contractRoot, name));
    const remote = await readFile(path.join(reference, name));
    if (!local.equals(remote)) throw new Error(`Publication-action mirror byte drift: ${name}`);
  }
}

console.log(`Publication-action contract mirror verified: ${expected.size} checksums${index === -1 ? "" : " + byte-identical reference"}`);
