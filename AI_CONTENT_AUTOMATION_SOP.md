...
- 客戶可先使用官網 `/rescue-request` 整理需求；該頁只在瀏覽器產生文字，不儲存、不送出，也不串接外部 API。
...
- CTA：文末固定電話與 LINE。
- 快速詢問：可連到 `/rescue-request`，提醒只填行政區，不填車牌、VIN、完整地址或姓名。
...
日期必須以當次對話／任務的實際日期明確帶入，不可直接信任執行環境時鐘。若系統時間與既有內容相差過大，工具會停止並要求明確日期，避免 sitemap 出現未來日期。
