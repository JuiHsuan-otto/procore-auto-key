...
- 新增官網 HTML 文章後，必須使用 `publish_tool.py` 同步 `blog.json`、`blog.html`、`cases.html`、`cases.json`、首頁最新案例與 `sitemap.xml`，不得手動硬改清單與 sitemap。
...
> **目前狀態：API 路徑已停用。** 依使用者決定，不再嘗試 GBP / My Business API 申請、OAuth 或自動發布；本節只規範可供人工貼上的草稿。除非使用者日後重新明確啟用，工具不得登入或呼叫 GBP API。
...
注意：目前自動化工具只產生草稿與 payload，不直接發布；GBP API 路徑維持停用。
